...
router.post("/contactanos", async (req, res) => {
  ...
  try {
    ...
    if (result.rowsAffected <= 0) {
      throw "No existe datos con esos parametros";
    }
...
router.get("/contactanos/:id", async (req, res) => {
  //get por id
  try {
    let data = { ...req.body, ...req.params };
    let pool = await sql.connect(config);
...
router.put("/contactanos/:id", async (req, res) => {
  //modificar por id
  try {
    let data = { ...req.body, ...req.params };
    let contactanos = new ContactanosModule(data);
...
router.delete("/contactanos/:id", async (req, res) => {
  //eliminar
  try {
    let data = { ...req.body, ...req.params };
    let contactanos = new ContactanosModule(data);
