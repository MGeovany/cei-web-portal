/* eslint-disable react/prop-types */
import React from 'react'

// home svg
export const Arrow = (props) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 330 330'
    width={props.width}
    height={props.height}
    style={{
      enableBackground: 'new 0 0 330 330'
    }}
    xmlSpace='preserve'
    fill='#ffffff'
    {...props}
  >
    <path d='M15 180h263.787l-49.394 49.394c-5.858 5.857-5.858 15.355 0 21.213C232.322 253.535 236.161 255 240 255s7.678-1.465 10.606-4.394l75-75c5.858-5.857 5.858-15.355 0-21.213l-75-75c-5.857-5.857-15.355-5.857-21.213 0-5.858 5.857-5.858 15.355 0 21.213L278.787 150H15c-8.284 0-15 6.716-15 15s6.716 15 15 15z' />
  </svg>
)

// social media

export const LinkdIn = (props) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 504.4 504.4'
    style={{
      enableBackground: 'new 0 0 504.4 504.4'
    }}
    xmlSpace='preserve'
    {...props}
  >
    <path d='M377.6.2H126.4C56.8.2 0 57 0 126.6v251.6c0 69.2 56.8 126 126.4 126H378c69.6 0 126.4-56.8 126.4-126.4V126.6C504 57 447.2.2 377.6.2zM168 408.2H96v-208h72v208zm-36.4-240c-20.4 0-36.8-16.4-36.8-36.8s16.4-36.8 36.8-36.8 36.8 16.4 36.8 36.8c-.4 20.4-16.8 36.8-36.8 36.8zm276.8 240H348V307.4c0-24.4-3.2-55.6-36.4-55.6-34 0-39.6 26.4-39.6 54v102.4h-60v-208h56v28h1.6c8.8-16 29.2-28.4 61.2-28.4 66 0 77.6 38 77.6 94.4v114z' />
  </svg>
)

export const Instagram = (props) => (
  <svg xmlns='http://www.w3.org/2000/svg' {...props}>
    <title>{'Instagram icon'}</title>
    <path d='M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913a5.885 5.885 0 0 0 1.384 2.126A5.868 5.868 0 0 0 4.14 23.37c.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558a5.898 5.898 0 0 0 2.126-1.384 5.86 5.86 0 0 0 1.384-2.126c.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913a5.89 5.89 0 0 0-1.384-2.126A5.847 5.847 0 0 0 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227a3.81 3.81 0 0 1-.899 1.382 3.744 3.744 0 0 1-1.38.896c-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421a3.716 3.716 0 0 1-1.379-.899 3.644 3.644 0 0 1-.9-1.38c-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 1 0 0-12.324zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405a1.441 1.441 0 0 1-2.88 0 1.44 1.44 0 0 1 2.88 0z' />
  </svg>
)

export const Facebook = (props) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    viewBox='0 0 29.94 29.94'
    style={{
      enableBackground: 'new 0 0 29.94 29.94'
    }}
    xmlSpace='preserve'
    {...props}
  >
    <path d='M27.56.684H2.383A2.38 2.38 0 0 0 0 3.064v23.813a2.381 2.381 0 0 0 2.383 2.379H27.56a2.382 2.382 0 0 0 2.38-2.379V3.064a2.382 2.382 0 0 0-2.38-2.38zm-7.435 8.483c-.619-.362-1.11-.648-1.727-.648-.604 0-1.12.151-1.384.405-.264.252-.395.74-.395 1.461v1.067h3.229l-.699 2.968h-2.53v9.862h-4.056V14.42H10.67v-2.968h1.895v-1.133c0-1.193.143-1.907.425-2.496.281-.587.826-1.241 1.584-1.611.757-.369 1.877-.555 3.036-.555 1.188 0 2.116.396 3.254.715l-.739 2.795z' />
  </svg>
)

// red color

export const HomeRed = (props) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width={1920.36}
    height={599.245}
    {...props}
  >
    <path
      data-name='Rect\xE1ngulo 182'
      fill='#e35961'
      d='M.36 0h1920v462H.36z'
    />
    <path
      data-name='Trazado 7'
      d='M0 572.741s395.353 67.374 588.3-12.716 210.169-140.439 448.2-42.005 378.051-44.806 616.083 28 267.777 26.718 267.777 26.718v-122.8L0 419Z'
      fill='#e35961'
    />
  </svg>
)

export const HeroBanner = (props) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width={'100%'}
    height={'100%'}
    fill='#ffffff'
    {...props}
  >
    <g data-name='Grupo 5' fill='#e35961'>
      <path data-name='Rect\xE1ngulo 182' d='M.359 0h1920v343H.359z' />
      <path
        data-name='Trazado 7'
        d='M0 469.314s395.353 70.69 588.3-13.342 210.169-147.351 448.2-44.073 378.051-47.011 616.083 29.382 267.777 28.033 267.777 28.033v-128.84L0 308.007Z'
      />
    </g>
  </svg>
)
